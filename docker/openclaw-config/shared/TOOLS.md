...
## Cross-Expert Consultation

Three tools for inter-expert communication. Prefer sync for quick lookups, async for substantial analysis.

- **consult_expert** (sync): Ask another expert and block until response (max 60s).
  - Params: `expert` (ron|ben|leslie|tom|ann|april), `question` (string), `priority` (normal|high)
  - `priority=normal` → Haiku (fast, factual lookups). `priority=high` → Sonnet (complex analysis).
  - Example: `consult_expert(expert="ben", question="How many active users in Springfield?")`
  - Use for simple factual lookups that won't take long.

- **request_consultation** (async): Queue a question and get a requestId back immediately.
  - Params: `expert` (ron|ben|leslie|tom|ann|april), `question` (string), `priority` (normal|high)
  - Example: `request_consultation(expert="ben", question="Generate a retention trend analysis for Q4")`
  - Use for substantial analysis. Continue working while waiting.

- **check_consultation**: Poll for an async consultation result.
  - Params: `requestId` (string)
  - Example: `check_consultation(requestId="abc-123")`
  - Returns status ("queued" | "processing" | "completed") and response when ready.

**Limits**: Max 3 hops deep, max 2 concurrent async per request, 60s sync timeout, no circular chains (A→B→A rejected).
